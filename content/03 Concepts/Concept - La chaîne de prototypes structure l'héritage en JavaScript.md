...
Tout objet en JS a une **propriété interne `[[Prototype]]`** (exposée via `__proto__` ou `Object.getPrototypeOf`). Cette propriété pointe vers un autre objet — son prototype — ou vers `null`.
...
**Concepts liés** :
- [[Concept - this en JavaScript dépend du site d'appel pas de la définition]] *(this dans une méthode = l'instance qui invoque, pas le proto)*
- [[Concept - Une closure capture son environnement lexical à la création]] *(les méthodes prototype + closures sont les deux mécanismes d'encapsulation)*
