...
tags:
  ...
  - level/beginner
title: >-
  Concept - Une seule référence mutable ou plusieurs immutables règle
  anti-data-race
slug: une-seule-reference-mutable-ou-plusieurs-immutables-regle-anti-data-race
excerpt: >-
  C'est le théorème central du borrow checker, parfois appelé **"aliasing XOR
  mutability"**. Il sous-tend la promesse principale de Rust : **safe
  concurrency without garbage collection**. Sans cette règle, tu n'aurais ni
  "pas de use-after-free" (parce qu'un alias survivrait au prop
oneLiner: >-
  À tout instant, sur une même valeur, le borrow checker n'autorise **qu'une
  seule référence mutable** (`&mut T`) **OU** **plusieurs références
  immutables** (`&T`) — jamais les deux à la fois — règle qui élimine les data
  races par construction, à la compilation.
related:
